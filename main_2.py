...
from PyQt5.QtWidgets import QApplication, QMainWindow, QAction, QActionGroup, QMessageBox, QFileDialog, QHeaderView, \
        ...
        try:
            ...
            if use_classify:
                ...
                result = detect_object(name_model, model_ai, image_to_process,
                                       config.shapes, threshold_set, threshold_box,
                                       conf, iou, max_det, agnostic_nms, mode)
            ...
            try:
                if step == Step.READ_TRIGGER:
                    ...
                    if len(code) > 0 and code.startswith("cmd"):
                        start_time = time.perf_counter()  # ⬅️ bắt đầu đo test time
                    ...
                    self.main_logger.info(step)
                    try:
                        self.camera.led_on()  # ✅ bật đèn Dino
                        time.sleep(0.015)  # ✅ stabilize
                        step = Step.PREPROCESS
                    except Exception as ex:
                        self.main_logger.error(f"LED ON fail: {ex}")
                        step = Step.ERROR
                    ...
                    self.main_logger.info(step)
                    try:
                        self.camera.led_off()  # ✅ tắt đèn ngay sau khi chụp
                    except Exception as ex:
                        self.main_logger.error(f"LED OFF fail: {ex}")
                    ...
                    result = detect_object(self.name_model, self.model_ai, mat_check,
                                           ...
                                           self.conf, self.iou, self.max_det, self.agnostic_nms, mode)
                    ...
                    try:
                        ...
                        if start_time is not None:
                            ...
                            self.main_logger.info(f"TEST TIME: {cycle_time:.2f} ms")
